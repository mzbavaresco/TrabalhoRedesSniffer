...
void copyIP6(IP6Header &ip6Header)
{
    for(int i=0;i<4;i++)
    {
        sourceIpAddress[numberOfPackets-1][i]      =  ip6Header.sourceIpAddr[i];

        destinationIpAddress[numberOfPackets-1][i] =  ip6Header.destIpAddr[i];
    }
}

void readIP6(IP6Header &ip6Header)
{
    if (fread(&ip6Header, sizeof(IP6Header), 1, logfile1) != 1)
    {
        printf("Could not read IP6 HEADER\n");
...
void readPacket()
{
    ...
    while(readPacketHeader()){
        ...
        {
            readIP6(ip6Header[numberOfPackets-1]);
            tempProtocol= (int)ip6Header[numberOfPackets-1].nextHeader;
            flag=true;
